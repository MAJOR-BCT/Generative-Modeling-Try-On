...
if __name__ == '__main__':
    ...
    #root represents the home root for the datasets 
    root = config.root

    # source dirs
    person_root = os.path.join(root, 'image')
    pose_root = os.path.join(root, 'pose')
    cloth_root= os.path.join(root, 'cloth')
    cloth_mask_root = os.path.join(root, 'cloth-mask')
    parse_root = os.path.join(root, 'image-parse')

    # target dirs
    # preprocessing to get sobel and palm information from the images 

    palmrgb_dst = os.path.join(root, 'palm-rgb')
    palmmask_dst = os.path.join(root, 'palm-mask')
    
    
    #location to save the gradient images for all images of the person 

    gradient_dst = os.path.join(root, 'image-sobel')
    os.makedirs(palmrgb_dst, exist_ok=True)
    os.makedirs(palmmask_dst, exist_ok=True)
    os.makedirs(gradient_dst, exist_ok=True)

    data_modes = ['test_pairs', 'train_pairs']

    
    for data_mode in data_modes:
        # target dirs
        cloth_align_dst = os.path.join(root,  data_mode, 'cloth')
        clothmask_align_dst = os.path.join(root, data_mode, 'cloth-mask')
        os.makedirs(cloth_align_dst, exist_ok=True)
        os.makedirs(clothmask_align_dst, exist_ok=True)

        align_factor = 1.0
        p_names, c_names = [], []
        with open(os.path.join(root, data_mode + '.txt'), 'r') as f:
        ...
        for i, imname in tqdm(enumerate(p_names)):
            ...
            #for getting the area with the cloth, and not the background 

            c_fg = np.where(parse_array!=0)
    ...
    # getting the edges in images with gradient of the images 

    print('preprocessing the person phase for getting the sobel of the images ')
    person_list = sorted([i for _,_,i in os.walk(person_root)][0])
    # print(person_list)
    for person_name in tqdm(person_list):
        ...
        if('.PNG' in person_name):
            ...
            gradientx_outfn = os.path.join(gradient_dst, person_name.replace('.png', '_sobelx.png'))
            gradienty_outfn = os.path.join(gradient_dst, person_name.replace('.png', '_sobely.png'))
        print(gradientx_outfn)
        print(gradienty_outfn)
